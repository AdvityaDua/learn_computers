...
function Avatar({ name, image }: { name: string; image?: string }) {
  ...
  if (image && image.startsWith("http")) {
    return <img src={image} alt={name} style={{ width: 30, height: 30, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />;
  }
  return (
    <div style={{
          ...
          {!loading && progress && tab === "submissions" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
              ...
              {progress.submissions.map((s) => (
                <div key={s.taskId} style={{ background: "var(--surface-soft)", borderRadius: "0.75rem", padding: "0.75rem 1rem", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem" }}>
                  <div>
                    ...
                    <div style={{ fontSize: "0.75rem", color: "var(--muted)", marginTop: 2 }}>
                      ...
                      <span style={{ marginLeft: "0.5rem" }}>{s.originalName}</span>
                    </div>
                  </div>
                  <div style={{ fontSize: "0.75rem", color: "var(--muted)", flexShrink: 0, textAlign: "right" }}>
                    <span className="admin-badge admin-badge-green">Submitted</span>
                    <div style={{ marginTop: 4 }}>{fmtDate(s.submittedAt)}</div>
                  </div>
                </div>
              ))}
            </div>
...
export function StudentsView() {
  ...
  useEffect(() => {
    fetchAdmin("/users")
      .then((r) => r.json())
      .then((data) => {
        const list = Array.isArray(data) ? data.filter((u: Student) => u.role === "student" || !u.role) : [];
        setStudents(list);
        // Fetch real completion % for each student in parallel
        ...
        {loading ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table className="admin-table">
              <thead>
                <tr><th>Student</th><th>Email</th><th>Joined</th><th style={{ minWidth: 160 }}>Progress</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {filtered.map((s) => {
                    ...
                    <tr key={s._id}>
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                          ...
                          <span style={{ fontWeight: 600, color: "var(--foreground)" }}>{s.fullName || "—"}</span>
                        </div>
                      </td>
