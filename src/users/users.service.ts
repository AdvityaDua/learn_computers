...
export class UsersService {
  ...
  async getLeaderboard(): Promise<LeaderboardRow[]> {
    const [users, lessonsAgg, quizzesAgg, assignmentsAgg, activitiesAgg] = await Promise.all([
      this.userModel.find().select('fullName email profileImage role createdAt').lean(),
    ...
    const lessonMap = new Map(lessonsAgg.map((row) => [String(row._id), row.count]));
    const quizMap = new Map(quizzesAgg.map((row) => [String(row._id), row.count]));
    const assignmentMap = new Map(assignmentsAgg.map((row) => [String(row._id), row.count]));
    const activityMap = new Map(activitiesAgg.map((row) => [String(row._id), row.count]));
    ...
    const rows = users
      .map((u) => {
        ...
        const contentCount = lessonCount + quizCount + assignmentCount + activityCount;

        const score = lessonCount * 8 + quizCount * 6 + assignmentCount * 5 + activityCount * 4;
  ...
  private aggregateByCreator(model: Model<LessonDocument | QuizDocument | AssignmentDocument | ActivityDocument>) {
