...
export class SchoolsService {
  ...
  async create(dto: CreateSchoolDto): Promise<SchoolDocument> {
    const existing = await this.schoolModel.findOne({ code: dto.code.toUpperCase() });
  ...
  async findAll(page = 1, limit = 20, search?: string) {
    ...
    const [items, total] = await Promise.all([
      this.schoolModel.find(query).sort({ createdAt: -1 }).skip((safePage - 1) * safeLimit).limit(safeLimit).lean(),
  ...
  async update(id: string, dto: Partial<CreateSchoolDto>): Promise<SchoolDocument> {
    const patch: any = { ...dto };
    ...
    const school = await this.schoolModel.findByIdAndUpdate(id, patch, { new: true, runValidators: true });
    if (!school) throw new NotFoundException('School not found');
    return school;
  }
