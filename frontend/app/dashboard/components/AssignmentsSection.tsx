...
import type { Chapter, Assignment, ContentLocation } from "./types";
...
export default function AssignmentsSection({
  ...
  const lessonsForChapter = useMemo(
    () =>
      chapterFilter
        ...
        : [],
    [assignments, contentMap, chapterFilter]
  );
  ...
  const filtered = useMemo(
    ...
    [assignments, contentMap, search, chapterFilter, lessonFilter]
  );

  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-6 animate-fade-up">
      <div className="mb-6">
        <h2 className="text-3xl font-black tracking-tight">Assignments</h2>
        <p className="theme-muted mt-1 text-sm">
          Stay on top of tasks with deadlines — click any card to open.
        </p>
      </div>
      ...
      <FilterBar
        ...
        noun={`assignment${filtered.length !== 1 ? "s" : ""}`}
      />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {filtered.map((a, idx) => {
          ...
          // Urgency color only for the thin top stripe — conveys deadline info
          const stripeColor = due.cls.includes("red")
            ? "var(--admin-danger)"
            : due.cls.includes("yellow")
            ? "var(--admin-warning, #f59e0b)"
            : due.cls.includes("green")
                ...
                <div className="p-6 md:p-7 flex flex-col flex-1">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--accent-soft)] shadow-inner transition-transform duration-500 group-hover:scale-110">
                      <ClipboardList size={24} className="accent-text" />
                    </div>
